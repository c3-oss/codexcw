---
name: codexcw-expert
description: Use for questions about working in or with the codexcw monorepo — its polyglot layout, the four-language API (Go, Rust, Node, Python), usage patterns, and the devbox/just workflow. Routes "how do I …" usage questions to the canonical examples.
...
Four independent, idiomatic implementations of the same contract — **no FFI
between them**:
...
- `just` recipes are language-namespaced: `go-*`, `rust-*`, `node-*`, `py-*`,
...
- Conventional Commits with a mandatory scope (commitlint). Scopes: `go`/`cli`,
  `core`, `node`, `py`, `ci`, `tooling`, `docs`, `repo`.
